...
GBuffer::GBuffer()
{
	...
	// Set vertex shader
	m_pVS = (ID3D11VertexShader*)ShaderManager::GetInstance()->GetShader("Shaders/VS_deferred.hlsl", D3D11_SHVER_VERTEX_SHADER);
	// Set input layout
	m_pInputLayout = (ID3D11InputLayout*)ShaderManager::GetInstance()->GetInputLayout("Shaders/VS_deferred.hlsl");
	// Set pixel shader
	m_pPS = (ID3D11PixelShader*)ShaderManager::GetInstance()->GetShader("Shaders/phong_deferred.hlsl", D3D11_SHVER_PIXEL_SHADER);
